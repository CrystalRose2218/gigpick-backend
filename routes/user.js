...
// PUT - apply to Gig (add to appliedGigs array) --------------------------------
router.put('/apply/', Utils.authenticateToken, (req, res) => {
  // validate the request
  if(!req.body.gigId){
    return res.status(400).json({
      message: "No gig specified"
    })
  }

  console.log(req.user._id)
  console.log(req.body.gigId)
  // add gigId to the appliedGigs field
  User.updateOne({
    _id: req.user._id
  }, {
    $push: {
      appliedGigs: req.body.gigId
    }
  })
  ...
  .then((user) => {
    ...
    res.json({
      message: "Gig application submitted"
    })
  })
  .catch(err => {
    console.log(err)
    res.status(500).json({
      messaage: "Problem applying to gig!"
    })
  })
})
...
router.get('/:id', Utils.authenticateToken, (req, res) => {
  ...
  User.findById(req.params.id).populate('appliedGigs')
    .then(user => {
      res.json(user)
    })
    .catch(err => {
